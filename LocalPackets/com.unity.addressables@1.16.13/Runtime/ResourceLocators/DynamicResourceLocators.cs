        ...
        internal void CreateDynamicLocations(Type type, IList<IResourceLocation> locations, string locName, string subKey, IResourceLocation mainLoc)
        {
            ...
            else
            {
                if (mainLoc.HasDependencies)
                    locations.Add(new ResourceLocationBase(locName, $"{mainLoc.InternalId}[{subKey}]", mainLoc.ProviderId, EnumLocalResourceMode.Disable, mainLoc.ResourceType, mainLoc.Dependencies.ToArray()));
                else
                    locations.Add(new ResourceLocationBase(locName, $"{mainLoc.InternalId}[{subKey}]", mainLoc.ProviderId, EnumLocalResourceMode.Disable, mainLoc.ResourceType));
            }
        }
    }
}
